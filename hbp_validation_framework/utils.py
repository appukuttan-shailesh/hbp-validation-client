...
def run_test_offline(model="", test_config_file="", skip_incapable=False):
    ...
    skip_incapable : boolean
        SciUnit checks if the model has all the required capabilities. If it does not, 
        and skip_incapable=False, then a `CapabilityError` is raised. Default is set to False.
    Note
    ----
    ...
    score = test.judge(model, skip_incapable=skip_incapable, deep_error=True)
...
def run_test(username="", password=None, environment="production", model="", test_instance_id="", test_id="", test_alias="", test_version="", storage_project_id="", register_result=True, client_obj=None, skip_incapable=False, **params):
    ...
    **params : list
    ...
    test_result_file = run_test_offline(model=model, test_config_file=test_config_file, skip_incapable=skip_incapable)
